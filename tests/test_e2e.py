...
def test_registration_adoption_and_chip(api, auth_token):
    logger.info("[End-to-End][POSITIVE] Registration, adoption and chiping")
    ...
    # Act 
    with allure.step("Чипируем кота от лица админа"):
        logger.info("Чипируем кота от лица админа")
        chip_resp = api.chip_cat(cat_id, auth_token)
    with allure.step("Получаем чипированного кота по Id"):
        logger.info("Получаем чипированного кота по Id")
    ...
    with allure.step("Проверяем наличие кода чипирования"):
        ...
        assert "chipCode" in chip_resp.json()
        assert chip_resp.json()["chipCode"].startswith("RU-STATE-")
    with allure.step("Проверяем поля в ответе"):
        ...
        assert get_resp["chipCode"] == chip_resp.json()["chipCode"]
