...
@allure.story("POST/cats with authorized")
...
@pytest.mark.contract
@allure.feature("Contract")
@allure.story("POST/cats Unauthorized")
...
@pytest.mark.contract
@allure.feature("Contract")
@allure.story("POST/cats duplicate name")
def test_create_cat_duplicate_name(api, openapi_validator, auth_token):
    ...
    with allure.step("Создаём первого кота"):
        ...
        resp1 = api.create_cat(payload1, token=auth_token)
    ...
    with allure.step("Пытаемся создать второго с тем же именем"):
        ...
        resp2 = api.create_cat(payload2, token=auth_token)
...
def test_create_cat_with_optional_fields_contract(api, openapi_validator, auth_token):
    ...
    with allure.step("Создаём кота с optional-полями"):
        ...
        resp = api.create_cat(payload, token=auth_token)
...
def test_create_cat_invalid_contract(api, openapi_validator, payload, description, auth_token):
