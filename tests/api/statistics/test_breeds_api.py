...
def test_stats_breeds_grouping(api):
    ...
    with allure.step("Создаём 3ех котов"):
        ...
        api.create_cat(build_cat_payload(age=1, breed="Test_breed_1"))
        api.create_cat(build_cat_payload(age=2, breed="Test_breed_1"))
        api.create_cat(build_cat_payload(age=3, breed="Test_breed_2"))
