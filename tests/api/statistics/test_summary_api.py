...
@allure.feature("API")
@allure.story("GET/stats/summary")
def test_stats_summary_counts_and_rate(api, auth_token):
    logger.info("[API] checking summary statistics")

    # Arrange 
    with allure.step("Создаём 3ех котов"):
        logger.info("Создаём 3ех котов")
        cat_1 = api.create_cat(build_cat_payload(age=1, breed="Bengal"), token=auth_token).json()["id"]
        cat_2 = api.create_cat(build_cat_payload(age=2, breed="Bengal"), token=auth_token).json()["id"]
        cat_3 = api.create_cat(build_cat_payload(age=3, breed="Sphynx"), token=auth_token).json()["id"]
    ...
    with allure.step("Обновляем данные 1го и 2го кота о владельце"):
        ...
        api.adopt_cat(cat_1, {"userId": user_id}, token=auth_token)
        api.adopt_cat(cat_2, {"userId": user_id}, token=auth_token)
